...
- A per-post inclusion/exclusion metabox — dropped by the maintainer as not worth the UI; curation is the type-level matrix plus the per-capability developer filters. A `## Optional` section in llms.txt is reachable through the `kntnt_ai_visibility_llms_sections`/`…_llms_txt` filters only.
