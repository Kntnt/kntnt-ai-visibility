...
- **Default scope:** every singular entry of a **`publicly_queryable` post type** gets a `.md` alternate (posts, pages and any publicly-queryable CPT – not an opt-in list). The **home** gets `/index.md` **if and only if the front page is a static page**. **Blog index, archives (date/taxonomy/author) and search results get no `.md` by default** – they are listings, not singular content, and widening to them is a later opt-in. So the provider's `match()`/identity covers singular publicly-queryable content plus the static-home `index.md` routing.
